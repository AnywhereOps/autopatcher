# Downloaded from https://github.com/autopkg/rtrouton-recipes/blob/90eb2447addd20cddc99e28709d274c79cc9fe55/AdobeCreativeCloud/XMLReader.py
...
class XMLReader(Processor):
    ...
    output_variables = {
        "xml_reader_output_variables": {"description": "Output variables per 'elements' supplied as input."},
    }
    ...
    def main(self):
        ...
        for query in elements:
            ...
            try:
                ...
                raise ProcessorError("XPath '%s' could not be found in XML file %s!" % (xpath, path))
            ...
            if "attributes" in query:
                ...
                for key, val in attributes.items():
                    ...
                    if self.env[val] is None:
                        raise ProcessorError("Could not find attribute named '%s' at XPath '%s'!" % (key, xpath))
            ...
            if "text" in query:
                ...
                self.output("Assigning value of '%s' to output variable '%s'" % (self.env[var], var))
